...
var RESOURCE_DOWNLOAD_TIMEOUT = process.env.RESOURCE_DOWNLOAD_TIMEOUT || 10 * 60;
...
server.onResourceReceived = function (req, res, response) {
	/*
	var Contentmatch = _.findWhere(response.headers, { name: 'Content-Disposition' });
	if(Contentmatch){
		console.log(Contentmatch.value+' -- Date: ' + new Date());
	}*/
...
server.checkIfJavascriptTimedOut = function(req, res) {
    ...
    if (!this.phantom || this.phantom.id !== req.prerender.phantomId) {
		...
		util.log('timeout -> '+timeout + ' : req.prerender.stage -> '+ req.prerender.stage);
		util.log('noJsExecutionInFirstSecond -> '+noJsExecutionInFirstSecond);
